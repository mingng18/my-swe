...
## 2024-09-08 - Zustand useShallow mapping pattern
**Learning:** When using Zustand's `useShallow` to prevent unnecessary re-renders during state mutations (like streaming events), returning an array of newly created objects inside the selector will defeat the shallow comparison because the object references are new every time.
**Action:** Instead of returning mapped arrays of objects, return a single `Record<string, primitive>` dictionary, which `useShallow` can successfully compare using keys and primitive values.
