...
## 2026-04-18 - Optimize Octokit Pagination Memory Efficiency
**Learning:** Using the spread operator inside `for await` loops (`results.push(...response.data)`) with `octokit.paginate.iterator()` creates unnecessary temporary arrays, intermediate memory allocations, and risks exceeding the maximum call stack size on large paginated datasets.
**Action:** Use `octokit.paginate(method, options, mapFn)` exclusively for safely mapping and reducing paginated elements chunk-by-chunk without manual loop-based accumulation.
