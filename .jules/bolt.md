...
**Action:** When a parent component only needs to pass data to children or check status, replace full-object subscriptions with granular property selectors (e.g., `state.threads[id]?.status`). Push the subscription for rapidly changing data (like the streaming events array) down to the specific leaf component that actually renders it.
