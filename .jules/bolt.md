...
## 2025-08-30 - Avoid .filter() full-array scans on chronologically sorted buffers\n**Learning:** When managing chronologically sorted stream event buffers with TTLs, using `.filter()` creates unnecessary intermediate arrays and traverses elements unnecessarily (O(N) operations + allocations). Since events append at the end, timestamps strictly increase.\n**Action:** Replaced `.filter()` with an early-exit `while` loop that finds the first unexpired event, and used in-place `.splice()` to eliminate allocations and optimize iteration.
