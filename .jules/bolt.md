...
## 2024-05-22 - Bounded Concurrency Chunking
**Learning:** For directories containing a massive number of files, iterating over them via `fs.promises.readdir` and mapping the files immediately into unbounded `Promise.all(files.map(async file => readFile(file)))` loops is a common Node.js performance anti-pattern. While `Promise.all` parallelizes I/O efficiently, firing thousands of `readFile` requests simultaneously causes the OS-level file descriptor limit to be exceeded (`EMFILE` errors), spikes V8 heap memory usage dramatically, and pollutes the microtask queue with unnecessary `async` wrappers.
**Action:** Always implement bounded concurrency. When adding new dependencies (like `p-limit`) is discouraged or restricted, manual chunking using array slicing (`for (let i = 0; i < files.length; i += LIMIT) { ... chunk.map(...) }`) is a lightweight, zero-dependency, and highly effective fallback to restrict in-flight promises. Additionally, remove the `async` wrapper from `.map` callbacks returning promises to avoid excessive microtask queueing.
