...
## 2025-02-28 - Optimize Promise.all over map in semantic-search\n**Learning:** The semantic search tool was using `for` loops to artificially batch `async` operations and calling `await Promise.all()` within the loop, causing a sequential block. This is a common performance pitfall where concurrency is arbitrarily limited, slowing down I/O bound operations by not properly overlapping requests.\n**Action:** Use `.map()` and pass the entire array of promises to a single `Promise.all()` to maximize parallel execution.
