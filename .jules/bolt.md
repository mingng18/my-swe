...
## 2026-09-14 - Zustand useShallow for nested arrays
**Learning:** When a React component subscribes to a specific array within a larger state object (like `state.threads[id].todos`), and the parent object (`thread`) is frequently updated (e.g. by streaming events), the component will re-render even if the array itself hasn't changed.
**Action:** Use `useShallow` when subscribing to nested arrays or objects within Zustand to prevent unnecessary re-renders when parent objects are mutated.
