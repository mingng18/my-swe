...
describe("closeGithubIssueTool", () => {
	...
	it("should return error if GITHUB_TOKEN is missing", async () => {
		...
		const resultJson = await closeGithubIssueTool.invoke(
			validArgs,
			validConfig as any,
		);
		const result = JSON.parse(resultJson as string);
	...
	it("should return error if result lacks url or number", async () => {
		process.env.GITHUB_TOKEN = "env_token";
		mockCloseGithubIssue.mockResolvedValue({
			state: "closed",
		});
		...
		expect(result.error).toBe(
			"Failed to close issue. No URL or number returned.",
		);
	});
});
