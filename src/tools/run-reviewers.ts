...
export const runReviewersTool = tool(
	async (args, config) => {
		...
		try {
			...
			// ⚡ Bolt: O(1) Map lookup instead of O(N*M) nested array search
			const builtInSubagentsMap = new Map(
				builtInSubagents.map((agent) => [agent.name, agent]),
			);
			...
			const promiseResults = await Promise.all(
				reviewerNames.map((reviewerName) => {
					const reviewerConfig = builtInSubagentsMap.get(reviewerName);
