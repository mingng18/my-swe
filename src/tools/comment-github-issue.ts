...
const logger = createLogger("comment-github-issue-tool");

/**
 * Post a comment to a GitHub issue.
 */
export const commentGithubIssueTool = tool(
  async ({ issue_number, body }, config) => {
    ...
    try {
      ...
      if (success) {
        logger.info({ issue_number }, "[comment_github_issue] Comment posted successfully.");
