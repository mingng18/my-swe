...
async function prewarmRepo(
  ...
  for (let i = 0; i < delta; i++) {
    ...
    await createAndCloneSandbox(owner, name, profile, threadId, apiKey, githubToken);
  }
}

async function createAndCloneSandbox(
  owner: string,
  name: string,
  profile: SandboxProfile,
  threadId: string,
...
async function main(): Promise<void> {
  ...
  for (const repo of repos) {
    await prewarmRepo(repo, defaultProfile, defaultCount, apiKey, githubToken);
  }
