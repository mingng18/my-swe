...
describe("sanitizeString", () => {
  ...
  it("should throw if input exceeds max length", () => {
    ...
    expect(() => sanitizeString(input, { maxLength: 10 })).toThrow(/Input too large/);
  });

  it("should strip control characters by default but throw when found", () => {
    // Note: the implementation says 'check for control characters BEFORE stripping' and throws
    const input = "hello\x00world";
    expect(() => sanitizeString(input)).toThrow(/Input contains null byte or control characters/);
  });
  ...
  it("should throw on dangerous patterns", () => {
    ...
    for (const input of dangerousInputs) {
      expect(() => sanitizeString(input)).toThrow(/Input contains potentially dangerous pattern/);
    }
  });

  it("should normalize unicode by default", () => {
    const unnormalized = "e\u0301"; // e + acute accent
    const result = sanitizeString(unnormalized);
    expect(result.value).toBe("\u00E9"); // normalized é
  });
  ...
  it("should enforce allowed characters regex", () => {
    expect(() => sanitizeString("hello!", { allowedChars: /^[a-z]+$/ })).toThrow(
      /Input contains characters not matching allowed pattern/
    );
...
describe("sanitizeUserPrompt", () => {
  it("should sanitize valid prompts", () => {
    expect(sanitizeUserPrompt("Please write a test")).toBe("Please write a test");
  });

  it("should throw on empty prompts", () => {
    expect(() => sanitizeUserPrompt("   ")).toThrow("User prompt cannot be empty");
  });
});
...
describe("sanitizeThreadId", () => {
  ...
  it("should throw on invalid characters", () => {
    expect(() => sanitizeThreadId("thread@123")).toThrow(/Input contains characters not matching/);
  });
});
...
describe("sanitizeBranchName", () => {
  it("should sanitize branch name", () => {
    expect(sanitizeBranchName("feature/new-button_1")).toBe("feature/new-button_1");
  });

  it("should throw on invalid characters", () => {
    expect(() => sanitizeBranchName("feature/new button")).toThrow(/Input contains characters not matching/);
  });
});
...
describe("sanitizeCommitMessage", () => {
  ...
  it("should throw on empty message", () => {
    expect(() => sanitizeCommitMessage("   ")).toThrow("Commit message cannot be empty");
  });

  it("should allow newlines", () => {
    expect(sanitizeCommitMessage("Fix bug\n\nDetails here")).toBe("Fix bug\n\nDetails here");
  });
});
...
describe("sanitizeUrl", () => {
  ...
  it("should throw on non-http/https protocols", () => {
    ...
    expect(() => sanitizeUrl("javascript:alert(1)")).toThrow(/Input contains potentially dangerous pattern/);
  });
...
describe("parseJsonSafely", () => {
  it("should parse valid JSON", () => {
    expect(parseJsonSafely<any>('{"hello": "world"}')).toEqual({ hello: "world" });
    expect(parseJsonSafely<any>('[1, 2, 3]')).toEqual([1, 2, 3]);
  });

  it("should throw on size limits", () => {
    expect(() => parseJsonSafely('{"a": 1}', { maxSize: 5 })).toThrow(/JSON input exceeds maximum size/);
  });
  ...
  it("should throw when exceeding max depth", () => {
    ...
    expect(() => parseJsonSafely(deepJson, { maxDepth: 2 })).toThrow(/JSON depth exceeds maximum/);
  });

  it("should block prototype pollution", () => {
    const polluted = '{"__proto__": {"polluted": true}}';
    expect(() => parseJsonSafely(polluted)).toThrow(/Prototype pollution detected/);

    const constructed = '{"constructor": {"prototype": {"polluted": true}}}';
    expect(() => parseJsonSafely(constructed)).toThrow(/Prototype pollution detected/);
  });
  ...
  it("should validate using schema", () => {
    ...
    expect(parseJsonSafely('{"name": "Alice", "age": 30}', { schema: TestSchema })).toEqual({
      name: "Alice",
      age: 30
    });

    expect(() => parseJsonSafely('{"name": "Alice"}', { schema: TestSchema })).toThrow();
  });
});
