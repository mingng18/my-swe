  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (trimmed.startsWith("export ")) {
      ...
      if (functionMatch) {
        ...
        seenSymbols.add(functionMatch[1]);
      } else if (classMatch) {
    ...
    if (funcMatch) {
      if (!seenSymbols.has(funcMatch[1])) {
        structure.symbols.push({
          name: funcMatch[1],
          type: "function",
          line: lineNumber,
    ...
    if (classMatch) {
      if (!seenSymbols.has(classMatch[1])) {
        structure.symbols.push({
          name: classMatch[1],
          type: "class",
          line: lineNumber,
    ...
    if (isTypeScript) {
      ...
      if (interfaceMatch) {
        ...
        seenSymbols.add(interfaceMatch[1]);
      }
