...
async function supabaseSelectSingle(
  table: string,
...
async function supabaseUpsertSingle(
  table: string,
  row: SupabaseRow,
): Promise<SupabaseRow | null> {
  const urlBase = getSupabaseUrlBase();
...
async function supabaseInsertMany(
  table: string,
  rows: SupabaseRow[],
): Promise<void> {
  const urlBase = getSupabaseUrlBase();
...
export async function writeRepoMemoryAfterAgentTurn(
  ...
  try {
    ...
    // Run these mostly independently
    // 1) Repo upsert -> repo_id (this still needs to be awaited since it provides resolvedRepoId for the others)
    ...
    // Prepare 2) Thread repo context upsert promise
    const threadRepoUpsertPromise = supabaseUpsertSingle(
      "thread_repo_context",
      {
    ...
    let agentRunPromise: Promise<unknown>;
    ...
    if (existingRun?.id && typeof existingRun.id === "string") {
      ...
      agentRunPromise = supabaseUpsertSingle("agent_run", {
        ...
        if (!inserted?.id || typeof inserted.id !== "string") {
          throw new Error("agent_run insert did not return id");
        }
      });
    }

    // We must wait for the agent run insert/update to complete before doing the facts,
    // or if we catch error, we bail out.
    try {
      await agentRunPromise;
    } catch (err) {
      logger.warn(
        { err },
        "[repo-memory] agent_run insert did not return id or failed; aborting facts",
      );
      await threadRepoUpsertPromise.catch(() => {}); // cleanup
      return;
    }
    ...
    const insertManyPromises: Promise<any>[] = [
      supabaseInsertMany("repo_memory_facts", fallbackFactRows),
    ];
    ...
    if (vectorEnabled && chunkRows) {
      ...
      insertManyPromises.push(
        supabaseInsertMany("repo_memory_chunks", fallbackChunkRows),
      );
    }

    // Wait for everything to settle
    await Promise.all([threadRepoUpsertPromise, ...insertManyPromises]);
