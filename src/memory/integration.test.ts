...
class MockSupabaseClient {
  ...
  async fetch(url: string, options?: RequestInit): Promise<Response> {
    ...
    const inMatch = url.match(/thread_id=in\.\(([^)]+)\)/);

    if (method === "GET") {
      if (threadIdMatch || inMatch) {
        let threadIds: string[] = [];
        if (inMatch) {
            threadIds = inMatch[1].split(',').map(decodeURIComponent);
        } else if (threadIdMatch) {
            threadIds = [decodeURIComponent(threadIdMatch[1])];
        }
  ...
  describe("Semantic Search", () => {
    it("should return relevant results for semantic queries", async () => {
      ...
      const results = await searchService.search({
        query: "React development", similarityThreshold: 0.1,
        threadIds: [threadId],
        limit: 5,
      });
    ...
    it("should filter by memory type", async () => {
      ...
      const results = await searchService.search({
        query: "TypeScript",
        threadIds: [threadId],
        types: ["user"], similarityThreshold: 0.1,
        limit: 5,
      });
