...
import {
	ThreadManager,
	THREAD_TTL_MS,
	threadManager as exportedThreadManager,
	threadRepoMap as exportedThreadRepoMap,
} from "../thread-manager";
...
describe("ThreadManager", () => {
	...
	describe("TTL and Eviction", () => {
		...
		it("should call disposal functions on sandbox eviction", async () => {
			const releaseRepoSandboxSpy = spyOn(
				daytonaPool,
				"releaseRepoSandbox",
			).mockResolvedValue();
		...
		it("should handle disposal failures gracefully", async () => {
			...
			const releaseErr = { name: "MockError", message: "Failed release" };
			const cleanupErr = { name: "MockError", message: "Failed cleanup" };
			...
			spyOn(daytonaPool, "releaseRepoSandbox").mockImplementation(() =>
				Promise.reject(releaseErr),
			);
			const mockBackendCleanup = mock(() => Promise.reject(cleanupErr));
			spyOn(sandboxState, "clearSandboxBackend");
			spyOn(toolInvocationTracker, "clearThread");
			...
			expect(sandboxState.clearSandboxBackend).toHaveBeenCalledWith(
				"thread-error",
			);
			expect(toolInvocationTracker.clearThread).toHaveBeenCalledWith(
				"thread-error",
			);

			expect(warnSpy).toHaveBeenCalledWith(
				{ error: releaseErr, threadId: "thread-error" },
				"[thread-manager] Failed to release old sandbox",
			);
			expect(warnSpy).toHaveBeenCalledWith(
				{ error: cleanupErr, threadId: "thread-error" },
				"[thread-manager] Failed to cleanup old backend",
			);
		});
		...
		it("should handle disposal failures gracefully during repo eviction", async () => {
			...
			const removePersistedThreadRepoSpy = spyOn(
				threadMetadataStore,
				"removePersistedThreadRepo",
			...
			expect(removePersistedThreadRepoSpy).toHaveBeenCalledWith(
				"thread-error-repo",
			);
			expect(warnSpy).toHaveBeenCalledWith(
				expect.objectContaining({ error: err, threadId: "thread-error-repo" }),
				"[thread-manager] Failed to remove persisted thread repo",
			);
		});
