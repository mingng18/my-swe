...
const mockInstallDependencies = mock(async () => ({ installed: true, packageManager: "npm", output: "" }));
const mockRunTests = mock(async () => ({ testPassed: true }));
const mockRunLinter = mock(async () => ({ lintPassed: true }));
const mockEnforcePRSubmission = mock(async () => ({ prCreated: true, prUrl: "http://pr", error: "" }));
...
describe("runVerificationPipeline", () => {
  beforeEach(() => {
    ...
    mockInstallDependencies.mockImplementation(async () => ({ installed: true, packageManager: "npm", output: "" }));
    mockRunTests.mockImplementation(async () => ({ testPassed: true }));
    mockRunLinter.mockImplementation(async () => ({ lintPassed: true }));
    mockEnforcePRSubmission.mockImplementation(async () => ({ prCreated: true, prUrl: "http://pr", error: "" }));
  });
