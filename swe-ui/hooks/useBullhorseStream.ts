...
export function useBullhorseStream({
  ...
  // ⚡ Bolt Optimization: Subscribe only to the required actions instead of the entire store
  // to prevent useBullhorseStream (and its consumers) from re-rendering on every store update.
